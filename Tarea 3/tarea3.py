...
class Controller(pyglet.window.Window):
    ...
    def update_escena(self,modelo1,modelo2):
        ...
        posicion_izq = sg.findPosition(modelo1, "hija1")
        posicion_der = sg.findPosition(modelo1, "hija2")
        posicion_centro = sg.findPosition(modelo1, "nave_nodriza")
        sombra_centro = sg.findNode(modelo2, "sombra")
        sombra_izq = sg.findNode(modelo2, "sombra2")
        sombra_der = sg.findNode(modelo2, "sombra3")
        ...
        sombra_centro.transform = tr.matmul([tr.translate(posicion_centro[0][0],0.2,posicion_centro[2][0]),tr.uniformScale(0.8)])
        sombra_izq.transform = tr.matmul([tr.translate(posicion_izq[0][0],0.2,posicion_izq[2][0]),tr.uniformScale(0.5)])
        sombra_der.transform = tr.matmul([tr.translate(posicion_der[0][0],0.2,posicion_der[2][0]),tr.uniformScale(0.5)])

    def mov_curva(self,modelo1,modelo2,C):
        posicion_izq = sg.findPosition(modelo1, "hija1")
        posicion_der = sg.findPosition(modelo1, "hija2")
        posicion_centro = sg.findPosition(modelo1, "nave_nodriza")
        sombra_centro = sg.findNode(modelo2, "sombra")
        ...
        largo = len(C)
        if self.step == largo - 1:
            self.step = 0

        angleY = np.arctan2(C[self.step+1,0]-C[self.step,0], C[self.step+1,2]-C[self.step,2]) - np.pi/2
        
        if C[self.step+1,2]-C[self.step,2] > 0:
            angleXZ = np.arctan2(C[self.step+1,1]-C[self.step,1], C[self.step+1,2]-C[self.step,2])#arreglar este angulo, anda medio loco
        else:
            angleXZ = np.arctan2(C[self.step+1,1]-C[self.step,1], C[self.step,2]-C[self.step+1,2])


        modelo1.transform = tr.matmul([tr.translate(C[self.step,0], C[self.step,1], C[self.step,2]), tr.rotationY(angleY), tr.rotationZ(angleXZ)])
        #transformaciones de las sombras.
        sombra_centro.transform = tr.matmul([tr.translate(posicion_centro[0][0],0.2,posicion_centro[2][0]),tr.uniformScale(0.8)])
        sombra_izq.transform = tr.matmul([tr.translate(posicion_izq[0][0],0.2,posicion_izq[2][0]),tr.uniformScale(0.5)])
        sombra_der.transform = tr.matmul([tr.translate(posicion_der[0][0],0.2,posicion_der[2][0]),tr.uniformScale(0.5)])
    ...
    def piruetas(self,modelo1,modelo2,C):     
        ...
        #if self.step < self.N2-1:  #revisar esto después.
        angleY = np.arctan2(C[self.step+1,0]-C[self.step,0], C[self.step+1,2]-C[self.step,2]) - np.pi/2
                
        modelo1.transform = tr.matmul([tr.translate(C[self.step,0], C[self.step,1], C[self.step,2]), tr.rotationY(angleY)])
        #transformaciones de las sombras.
        sombra_centro.transform = tr.matmul([tr.translate(posicion_centro[0][0],0.2,posicion_centro[2][0]),tr.uniformScale(0.8)])
        sombra_izq.transform = tr.matmul([tr.translate(posicion_izq[0][0],0.2,posicion_izq[2][0]),tr.uniformScale(0.5)])
        sombra_der.transform = tr.matmul([tr.translate(posicion_der[0][0],0.2,posicion_der[2][0]),tr.uniformScale(0.5)])
    ...
    def dibujar(self,pipeline,C):
        lineas = {}
        for i in range(0,len(C)-1):
            lineas[str(i)] = createGPUShape(pipeline,bs.crearlinea(C[i],C[i+1],0.8,0.8,0.1))
        return lineas
...
def setPlot(texPipeline, axisPipeline, lightPipeline,posicionX,posicionY,posicionZ,camara):

    if camara == False:
        projection = tr.ortho(-8, 8, -8, 8, 0.1, 1000) #camara ortografica
    else:
        projection = tr.perspective(70, 1, 0.1, 100) #camara perspectiva
    ...
    glUniform3f(glGetUniformLocation(lightPipeline.shaderProgram, "lightPosition"), posicionX,posicionY+15,posicionZ)
...
ASSETS = {
    ...
    "camara": getAssetPath("camara.obj"),
}
...
def createScene(pipeline):
    ...
    camara = createGPUShape(pipeline, readOBJ(ASSETS["camara"],(1,1,1)))
    ...
    camaraNode = sg.SceneGraphNode("camara")
    camaraNode.transform = tr.matmul([
                                tr.translate(-10,5,0),
                                tr.uniformScale(0.1),
    ])
    camaraNode.childs += [camara]
...
def crear_curva(lista):
    curvas = []
    for i in range(0,len(lista)-1):
        L1 = lista[i]
        L2 = lista[i+1]
        #Puntos de control
        P1 = np.array([[L1[0], L1[1], L1[2]]]).T
        P2 = np.array([[L2[0], L2[1], L2[2]]]).T
        #Tangentes
        #T1 = np.array([[np.cos(L1[3]) * np.sin(L1[4]), np.sin(L1[3]) * np.sin(L1[4]), np.cos(L1[4])]]).T
        #T2 = np.array([[np.cos(L2[3]) * np.sin(L2[4]), np.sin(L2[3]) * np.sin(L2[4]), np.cos(L2[4])]]).T

        T1 = 7*np.array([[np.sin(L1[3]) * np.sin(L1[4]), np.cos(L1[4]), np.sin(L1[4]) * np.cos(L1[3])]]).T
        T2 = 7*np.array([[np.sin(L2[3]) * np.sin(L2[4]), np.cos(L2[4]), np.sin(L2[4]) * np.cos(L2[3])]]).T

        M = hermiteMatrix(P1, P2, T1, T2)
        curvas.append(evalCurve(M, controller.N))
    
    #concatenamos todas las curvas.
    C = np.concatenate((curvas), axis=0)
    return C

#CÁMARA LISTA.
def update_camara(modelo1):
    #camara normal.
    if controller.camara2 == False:
        posicion_nave = sg.findPosition(modelo1, "naves")
        controller.viewPos = np.array([posicion_nave[0][0],posicion_nave[1][0],posicion_nave[2][0]]) + np.array([-10,10,10]) #arreglar esto
        controller.at = np.array([posicion_nave[0][0],posicion_nave[1][0],posicion_nave[2][0]])
        #Camara en 3ra persona.
    else:
        posicion_nave = sg.findPosition(modelo1, "naves")
        posicion_camara = sg.findPosition(modelo1, "camara")
        controller.viewPos = np.array([posicion_camara[0][0],posicion_camara[1][0],posicion_camara[2][0]])
        controller.at = np.array([posicion_nave[0][0],posicion_nave[1][0],posicion_nave[2][0]])
...
def on_key_press(symbol, modifiers):
    ...
    #Control puntos
        ...
        #guardamos puntos manualmente.
        posicion_nave = sg.findPosition(escena, "naves")
        controller.puntos.append((posicion_nave[0][0],posicion_nave[1][0],posicion_nave[2][0],controller.anguloXZ,controller.anguloY))
        controller.cantidad_puntos += 1
        print("-----------------------------------")
        print("Punto agregado: " + "(" + str(posicion_nave[0][0]) + "," + str(posicion_nave[1][0]) + "," + str(posicion_nave[2][0]) + ")")
        print("Orientacion de la nave: " + "AnguloXZ: " + str(controller.anguloXZ) + " , " + "AnguloY: " + str(controller.anguloY))
        print("Total de puntos: " + str(controller.cantidad_puntos))
    
    #Ver curvas.
    elif symbol == pyglet.window.key.V: #ver curvas
        if len(controller.puntos)<=1:
            print("No hay suficientes puntos para mostrar la curva.")
        else:  
            controller.curva = crear_curva(controller.puntos)
            controller.lineas = controller.dibujar(axisPipeline,controller.curva)
            controller.mostrar_curva = not(controller.mostrar_curva)

    #Empezar el movimiento.
    elif symbol == pyglet.window.key._1: 
        if len(controller.puntos)<=1:
            print("No hay suficientes puntos para empezar el movimiento.")
        else:
            controller.curva = crear_curva(controller.puntos)
            controller.movimiento = not(controller.movimiento)
            controller.lineas = controller.dibujar(axisPipeline,controller.curva)
    #pirueta.
    elif symbol == pyglet.window.key.P:
        controller.punto_pirueta = []
        posicion_nave = sg.findPosition(escena, "naves")
        controller.punto_pirueta.append((posicion_nave[0][0],posicion_nave[1][0],posicion_nave[2][0]))
        curvas = []
        L1 = controller.punto_pirueta[0]
        P0 = np.array([[L1[0], L1[1], L1[2]]]).T
        P1 = np.array([[L1[0]+1, L1[1], L1[2]-1]]).T
        T0 = np.array([[1,0,0]]).T
        T1 = np.array([[0,0,-1]]).T
        M1 = hermiteMatrix(P0,P1,T0,T1)
        curvas.append(evalCurve(M1, 10))

        P0 = np.array([[L1[0]+1, L1[1], L1[2]-1]]).T
        P1 = np.array([[L1[0], L1[1], L1[2]-2]]).T
        T0 = np.array([[0,0,-1]]).T
        T1 = np.array([[-1,0,0]]).T
        M2 = hermiteMatrix(P0,P1,T0,T1)
        curvas.append(evalCurve(M2, 10))

        P0 = np.array([[L1[0], L1[1], L1[2]-2]]).T
        P1 = np.array([[L1[0]-1, L1[1], L1[2]-1]]).T
        T0 = np.array([[-1,0,0]]).T
        T1 = np.array([[0,0,1]]).T
        M3 = hermiteMatrix(P0,P1,T0,T1)
        curvas.append(evalCurve(M3, 10))

        P0 = np.array([[L1[0]-1, L1[1], L1[2]-1]]).T
        P1 = np.array([[L1[0], L1[1], L1[2]]]).T
        T0 = np.array([[0,0,1]]).T
        T1 = np.array([[1,0,0]]).T
        M4 = hermiteMatrix(P0,P1,T0,T1)
        curvas.append(evalCurve(M4, 10))

        controller.pirueta = np.concatenate((curvas), axis=0)

        controller.mov_pir = not(controller.mov_pir)
...
def on_draw():
    ...
    #movimientos manuales de la escena.
    if controller.mov_pir == False:
        if controller.movimiento == False:
            controller.update_escena(escuadron,sombras)
        else:
            controller.mov_curva(escuadron,sombras,controller.curva)
    else:
        controller.piruetas(escuadron,sombras,controller.pirueta)
    
    #distintas cámaras.
    update_camara(escuadron)

    if controller.mostrar_curva == True:
        for i in controller.lineas:
            glUseProgram(axisPipeline.shaderProgram)
            axisPipeline.drawCall(controller.lineas[i],GL_LINES)
    ...
    #la iluminación se mueve con la nave.
    setPlot(texPipeline, axisPipeline, lightPipeline,posicion_nave[0][0],posicion_nave[1][0],posicion_nave[2][0],controller.camara2)
